...
master_ip="192.168.55.214"
'''
sock = socket.socket(socket.AF_INET, socket.SOCK_DGRAM)
server_address = ('192.168.55.214', 10010)

sock_tx1 = socket.socket(socket.AF_INET, socket.SOCK_DGRAM)  #...send to gps data to GCS
server_address_tx1 = ('192.168.55.214', 13697)
...
server_address1 = ('192.168.55.214', 12008)
...
udp_socket = socket.socket(socket.AF_INET, socket.SOCK_DGRAM)
share_data_server_address1 = ('192.168.55.214', 12008)
udp_socket2 = socket.socket(socket.AF_INET, socket.SOCK_DGRAM)
...
server_address11 = ('192.168.55.214', 12002)
...
file_server_address = ('192.168.55.214', 12003)  
...
mavlink_server_address1 = ('192.168.55.214', 12045)
...
fileloc="/home/dhaksha/Documents/socket_plane/dce_swarm_nav/swarm_tasks-main/swarm_tasks/Examples/basic_tasks/"
...
def CHECK_network_connection_odd(slave_host_odd, wait_time=None):
    ...
    while True:
        ...
        if response==0: # Link is OK.
            ...
            if down_counter > 2:
		if slave_host_odd == '192.168.55.214':
...
def CHECK_network_connection_even(slave_host_even, wait_time=None):
    ...
    while True:
        ...
        if response==0: # Link is OK.
            ...
            print('{} - Connection to router is DOWN for {} times.'.format(time.ctime(), down_counter))
            if down_counter > 2:
		if slave_host_even == '192.168.55.214':
...
def vehicle_connection():
    ...
    try:
        ...
        vehicle1 = connect('udpin:192.168.55.214:14551', baud=115200, heartbeat_timeout=30)
    ...
    try:
        vehicle2 = connect('udpin:192.168.55.214:14552', baud=115200, heartbeat_timeout=30)
	...
	uavs_array.append('uav2')
        ...
        #time.sleep(0.5)
    try:
        vehicle3 = connect('udpin:192.168.55.214:14553', baud=115200, heartbeat_timeout=30)
    ...
    try:
        vehicle4 = connect('udpin:192.168.55.214:14554', baud=115200, heartbeat_timeout=30)
        ...
        #time.sleep(0.5)
    try:
        vehicle5 = connect('udpin:192.168.55.214:14555', baud=115200, heartbeat_timeout=3)
        ...
        #time.sleep(0.5)
    try:
        vehicle6 = connect('udpin:192.168.55.214:14556', baud=115200, heartbeat_timeout=3)
        ...
        #time.sleep(0.5)
    try:
        vehicle7 = connect('udpin:192.168.55.214:14557', baud=115200, heartbeat_timeout=3)
        ...
        #time.sleep(0.5)
    try:
        vehicle8 = connect('udpin:192.168.55.214:14558', baud=115200, heartbeat_timeout=3)
        ...
        #time.sleep(0.5)
    try:
        vehicle9 = connect('udpin:192.168.55.214:14559', baud=115200, heartbeat_timeout=3)
        ...
        #time.sleep(0.5)
    try:
        vehicle10 = connect('udpin:192.168.55.214:14560', baud=115200, heartbeat_timeout=3)
    ...
    try:
        vehicle11 = connect('udpin:192.168.55.214:14561', baud=115200, heartbeat_timeout = 0.5)
        ...
        #time.sleep(0.5)
    try:
        vehicle12 = connect('udpin:192.168.55.214:14562', baud=115200, heartbeat_timeout = 0.5)
    ...
    try:
        vehicle13 = connect('udpin:192.168.55.214:14563', baud=115200, heartbeat_timeout = 0.5)
        ...
        #time.sleep(0.5)
    try:
        vehicle14 = connect('udpin:192.168.55.214:14564', baud=115200, heartbeat_timeout = 0.5)
        ...
        #time.sleep(0.5)
    try:
        vehicle15 = connect('udpin:192.168.55.214:14565', baud=115200, heartbeat_timeout = 0.5)
    ...
    """
    try:
        vehicle16 = connect('udpin:192.168.55.214:14566', baud=115200, heartbeat_timeout = 0.5)
        ...
        #time.sleep(0.5)
    try:
        vehicle17 = connect('udpin:192.168.55.214:14567', baud=115200, heartbeat_timeout = 0.5)
        ...
        #time.sleep(0.5)
    try:
        vehicle18 = connect('udpin:192.168.55.214:14568', baud=115200, heartbeat_timeout = 0.5)
        ...
        #time.sleep(0.5)
    try:
        vehicle19 = connect('udpin:192.168.55.214:14569', baud=115200, heartbeat_timeout = 0.5)
    ...
    try:
        vehicle20 = connect('udpin:192.168.55.214:14570', baud=115200, heartbeat_timeout = 0.5)
    ...
    try:
        vehicle21 = connect('udpin:192.168.55.214:14571', baud=115200, heartbeat_timeout = 0.5)
        ...
        #time.sleep(0.5)
    try:
        vehicle22 = connect('udpin:192.168.55.214:14572', baud=115200, heartbeat_timeout = 0.5)
        ...
        #time.sleep(0.5)
    try:
        vehicle23 = connect('udpin:192.168.55.214:14573', baud=115200, heartbeat_timeout = 0.5)
        ...
        #time.sleep(0.5)
    try:
        vehicle24 = connect('udpin:192.168.55.214:14574', baud=115200, heartbeat_timeout = 0.5)
    ...
    try:
        vehicle25 = connect('udpin:192.168.55.214:14575', baud=115200, heartbeat_timeout = 0.5)
...
def update_plot():	
	...
	uav_positions = OrderedDict((uav, []) for uav in uavs_array	)
	if filename=="":
		messagebox.showwarning("No Filename Selected", "Please select a Airport name.")
	print("uav_positions",uav_positions)
	uav_trajectories = OrderedDict()
	for i, uav in enumerate(uavs_array):
	    if uav in uav_colors:
	...
	def update_plot1(canvas):
		...
		while True:
			...
			try:
				...
				if (data==b"start") or (data==b"aggregate") or (data==b"return") or (data==b"same altitude") or (data==b"different altitude") or (data==b"disperse") or (data==b"stop")  or(data==b"search") or (data==b"circle formation") or (data.startswith(b"Drone"))or (data.startswith(b"vehicle")) or (data.startswith(b"Vehicle")) or (data.startswith(b"master_num"))or (data.startswith(b"pos_array")) or (data.startswith(b"Drone")) or (data.startswith(b"home")) or (data=="rtl") or (data==b"goal") or (data==b"skip_wp") or (data==b"CSV Cleared") or (data.startswith(b"remove_bot")) or (data.endswith(b"vehicle removed")):
				...
				if(data.startswith(b"home_pos")):
					...
					#continue
			        if(data.startswith(b"circle_latlon")):
					message_with_array = data.decode()
					message = message_with_array[:13]  # Assuming "home_pos" is 8 characters long
					array_data = message_with_array[13:]	
					# Deserialize the array data
					home_pos = json.loads(array_data)
					terminal_message_entry.insert(tk.END,str(data)+'\n')
					terminal_message_entry.see(tk.END)
					root.update()
					print("Received message:",message)
					print("Received array:circle_latlon", circle_latlon)
...
def skip_wp():
	print("STOP>>>>>>>>>>>>")
        global socket1,udp_socket,udp_socket2,server_address1,server_address2      
        socket1 = socket.socket(socket.AF_INET, socket.SOCK_DGRAM)
        data1 = skip_wp_entry.get()
        data = "skip_wp"+","+str(data1)
        print("data",data)
...
def Reconnect():
    ...
    connection_string = "udpin:192.168.55.214:"+str(Reconnect_port)
...
def mavlink_add():
    ...
    master_ip_array=['192.168.55.214','192.168.6.152','192.168.6.153','192.168.6.154','192.168.6.155','192.168.6.156','192.168.6.157','192.168.6.158','192.168.6.159','192.168.6.160']
...
def select_direction():
	    ...
	    select_label = tk.Label(root, text="Circle Direction")
	    ...
	    select_button = tk.Button(root, text="Select", command=select_direction)
	    select_button.place(x=700, y=600)
	    if str(radio_var) == "clockwise":
		dir_value=0
	    else:
		dir_value=1
	    '''
...
if __name__ == "__main__" : 
  ...
  #  frame_5 = Frame(height = 120, width = 300-70, bd = 3, relief = 'groove').place(x = 1682, y = 930)
    ...
    Button(text = "AUTO", command = auto_all, width = 5, font=("Times New Roman", 13)).place(x = 490+40, y = 950)
   ...
   # Button(text = "Resume", command = resume_socket, width = 6, font=("Times New Roman", 13)).place(x = 860+70+15+10, y = 1020)
    ...
    Button(text = "NAV", command = start1_socket, width = 6, font=("Times New Roman", 13)).place(x = 610, y = 950)
...
# Create a Label widget to display the area covered
    area_covered_label = tk.Label(root, textvariable=area_covered_var, font=("Times New Roman", 13)).place(x=15, y=850)
    ...
    Button(text = "Skip_wp", command = skip_wp, width = 10, font=("Times New Roman", 13)).place(x = 1600, y= 950)
    Button(text = "Target", command = goal_socket, width = 5, font=("Times New Roman", 13)).place(x = 1500, y= 950)
    Button(text = "Circle dir", command = select_direction, width = 7, font=("Times New Roman", 13)).place(x = 1400, y= 950)
    goal_entry= Entry()
    ...
    skip_wp_entry= Entry()
    skip_wp_entry.place(x = 1600, y= 990)
    #Button(text = "Enter", command = enter, width = 8, font=("Times New Roman", 13)).place(x = 1730+20+35, y= 890+120)
    terminal_msg_label= Label(text = "Message", font=("Times New Roman", 13),fg='blue').place(x = 450, y= 670)
    #terminal_message_entry=Entry()
    terminal_message_entry=Text(root)
    terminal_message_entry.place(x = 450, y= 700,width=450,height=200)
    ...
    Button(text = "Loiter Unlim", command = getlatlon, width = 9, font=("Times New Roman", 13)).place(x = 752+820, y = 1030)
    GET_entry = Entry()
    GET_entry.place(x = 785+900, y = 1035, width = 140, height = 25)
    ...
    def select_location():
	    ...
	    #update_plot()
	    fileloc="/home/casr-3/Documents/meharbaba/swarm_tasks/envs/worlds/"
	    print("fileloc",fileloc)
    ...
    # Location Options
    location_options = {
	    "Tambaram", "dce","Sholavaram","Lucknow","Jammu","Medur"
	}
    ...
    Button(text = "Remove Bot =", command = bot_remove, width = 10, font=("Times New Roman", 13)).place(x =752+650, y = 1025)
    removebot_entry = Entry()
    removebot_entry.place(x = 752+770, y = 1030,width=45)
