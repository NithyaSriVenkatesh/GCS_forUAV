...
def vehicle_connection():
    ...
    try:
        ...
        vehicle1 = connect('udpin:192.168.29.69:14551', baud=115200, heartbeat_timeout=3)
	...
	uavs_array.append('uav2')
        ...
        #time.sleep(0.5)
    try:
        vehicle3 = connect('udpin:192.168.29.69:14553', baud=115200, heartbeat_timeout=30)
    ...
    try:
        ...
        #time.sleep(0.5)
    try:
        vehicle5 = connect('udpin:192.168.29.69:14555', baud=115200, heartbeat_timeout=30)
    ...
    try:
        ...
        #time.sleep(0.5)
    try:
        vehicle9 = connect('udpin:192.168.29.69:14559', baud=115200, heartbeat_timeout=3)
...
csv_files = {
    'UAV1': '/home/dhaksha/Downloads/dec-3-drone-new/grid-csv/dec_3_d1.csv',
    'UAV2': '/home/dhaksha/Downloads/dec-3-drone-new/grid-csv/dec_3_d2.csv',
    'UAV3': '/home/dhaksha/Downloads/dec-3-drone-new/grid-csv/dec_3_d3.csv',
    # Add more UAVs and their corresponding CSV file paths as needed
}
...
def update_plot():	
	...
	def update_plot1(canvas):
		count=0
		global area_covered_var,fig,ax,fileloc,filename,filepath,uav_trajectories,uav_colors,uav_positions,follower_host_tuple,goal_table,return_goal_table,grid_path_table
		while True:
			
			#print("filepath",filepath)
			try:
				data, address = sock.recvfrom(1024)
				if (data==b"start") or (data==b"aggregate") or (data==b"return") or (data==b"same altitude") or (data==b"different altitude") or (data==b"disperse") or (data==b"stop")  or(data==b"search") or (data==b"circle formation") or (data.startswith(b"Drone"))or (data.startswith(b"vehicle")) or (data.startswith(b"Vehicle")) or (data.startswith(b"master_num"))or (data.startswith(b"pos_array")) or (data.startswith(b"Drone")) or (data.startswith(b"home")) or (data.startswith(b"Data")) or (data=="rtl") or (data==b"goal") or (data==b"specific_bot_goal") or (data==b"CSV Cleared") or (data.startswith(b"remove_bot")) or (data.endswith(b"vehicle removed")):
				...
				if data.startswith(b"search,"):					
					...
					print("grid_path_array_str",grid_path_array_str)
					  # Extract the array part
					grid_path_table = json.loads(grid_path_array_str)
					print("grid_path_table",grid_path_table)
				...
				print("grid_path_table",grid_path_table)
				if a[-1].strip().startswith(b'grid_path'):
				# Remove the last element (which is "path4")
				        last_element = a.pop()
					print("last_element",last_element,type(last_element))
					if last_element.strip().startswith(b'grid_path'):
						print("Removed element:", last_element[9:])
						h=int(last_element[9:])
						if h not in return_goal_table:
							print("h$$$$$$$$$$$", h)
							grid_path_table.append(h)
						print("grid_path_table",grid_path_table)
					print("grid_path_table",grid_path_table)
				print("return_goal_table,grid_path_table>>>>>>>>>!!!!!!!!!!!!!!!",goal_table,return_goal_table,grid_path_table)
...
if __name__ == "__main__" : 
  ...
  #  frame_5 = Frame(height = 120, width = 300-70, bd = 3, relief = 'groove').place(x = 1682, y = 930)
   ...
   # Button(text = "Resume", command = resume_socket, width = 6, font=("Times New Roman", 13)).place(x = 860+70+15+10, y = 1020)
    ...
    data="0.0"
    flight_time_var.set("Flight Time = " + data)  # Initialize with default value
    flight_time_label = tk.Label(root, textvariable=flight_time_var, font=("Times New Roman", 15),fg="blue").place(x=15, y=650)
...
# Create a Label widget to display the area covered
    area_covered_label = tk.Label(root, textvariable=area_covered_var, font=("Times New Roman", 15),fg="blue").place(x=15, y=850)
    ...
    Button(text = "Specific_Bot_Goal", command = specific_bot_goal_socket, width = 20, font=("Times New Roman", 13)).place(x = 1600, y= 950)
